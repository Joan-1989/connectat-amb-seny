...
const NavButton: React.FC<NavButtonProps> = ({ label, icon, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center justify-center w-1/5 p-2 transition-colors duration-200 ${
      isActive ? 'text-brand-primary' : 'text-gray-500 hover:text-brand-primary'
    }`}
    aria-pressed={isActive}
    aria-label={`Obrir mòdul ${label}`}
  >
    {icon}
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white shadow-lg border-t border-gray-200 flex z-20">
      ...
      />
        ...
        label="Progrés"
        icon={<InfoIcon />}
        isActive={activeModule === "Progrés"}
        onClick={() => setActiveModule("Progrés")}
      />
      <NavButton
        label="Perfil"
        icon={<ProfileIcon />}
